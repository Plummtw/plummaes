...
fn enbase32_file(input: &str, output: &str, key: &[u8]) {
    ...
    let bytes = input_bytes.chunks(8).flat_map(base32::encrypt_base32)
        .collect::<Vec<_>>();
...
fn debase32_file(input: &str, output: &str, key: &[u8]) {
    match std::fs::read(input) {
        Ok(bytes) => {
            ...
            let result : Vec<u8> = result.chunks(5).flat_map(base32::decrypt_base32).collect::<Vec<_>>();
...
fn debase32_file_toconsole(input: &str, key: &[u8]) {
    match std::fs::read(input) {
        Ok(bytes) => {
            // Check Header
            if String::from_utf8(bytes[0..8].to_vec()).unwrap() != "Plumm1.4" {
            ...
            let result : Vec<u8> = result.chunks(5).flat_map(base32::decrypt_base32)
                .collect::<Vec<_>>();
...
fn main() {
    ...
    if args.len() < 3 && args.len() > 5 {
        show_usage();
        // return;    
    } else if args.len() == 5 && args[1].to_lowercase() == "encrypt" {
    ...
    } else if args.len() == 4 && args[1].to_lowercase() == "debase32" {
        debase32_file_toconsole(&args[2], &key::read_key_file(&args[3]));
