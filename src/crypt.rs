...
pub(crate) fn encrypt(input: &[u8], key: &[u8]) -> Vec<u8> {
  ...
  let mut blocks = input_copy.chunks(16).map(
      GenericArray::clone_from_slice).collect::<Vec<_>>();

  let cipher = Aes256::new(key);
  cipher.encrypt_blocks(&mut blocks);
  let mut header_block = [0u8; 16];
  let len_blocks = input.len().to_be_bytes();
  let mut ret = blocks.iter().flat_map(|x| x.iter().cloned().collect::<Vec<_>>()).collect::<Vec<_>>();
...
pub(crate) fn decrypt(input: &[u8], key: &[u8]) -> Vec<u8> {
  ...
  for item in input.iter().take(16).skip(8) {
      length <<= 8;
      length |= *item as usize;
  }
  let key = GenericArray::from_slice(key);
